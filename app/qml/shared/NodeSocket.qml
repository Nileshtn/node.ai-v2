import QtQuick
import QtQuick.Controls
import "../theme"

Item {
    id: root

    signal connectionDragStarted(string socketId, string socketSide, real canvasX, real canvasY)
    signal connectionDragged(real canvasX, real canvasY)
    signal connectionDragFinished(real canvasX, real canvasY)

    property string socketId: ""
    property string displayLabel: ""
    property string side: "left"
    property Item nodeRoot: null
    readonly property string shownLabel: displayLabel.length > 0 ? displayLabel : socketId
    readonly property AppColors colors: AppColors {}
    readonly property int edgeHitBleed: 12
    readonly property int rowHitBleed: 3

    width: 96
    height: 26

    function connectorScenePosition() {
        return socketDot.mapToItem(null, socketDot.width / 2, socketDot.height / 2)
    }

    function connectorCanvasPosition() {
        return pointInGraphCanvas(socketDot, socketDot.width / 2, socketDot.height / 2)
    }

    function pointInGraphCanvas(item, localX, localY) {
        if (nodeRoot && nodeRoot.graphCanvas) {
            return nodeRoot.graphCanvas.mapFromItem(item, localX, localY)
        }

        var globalPoint = item.mapToItem(null, localX, localY)

        return {
            "x": globalPoint.x,
            "y": globalPoint.y
        }
    }

    Rectangle {
        id: socketDot

        width: 12
        height: 12
        radius: 6
        anchors.verticalCenter: parent.verticalCenter
        x: root.side === "left" ? -width / 2 : parent.width - width / 2
        color: colors.nodeSocket
        border.color: colors.nodeBackground
        border.width: 1
    }

    MouseArea {
        id: socketDragArea

        objectName: "socketConnectorDrag"
        anchors.fill: parent
        anchors.topMargin: -root.rowHitBleed
        anchors.bottomMargin: -root.rowHitBleed
        anchors.leftMargin: root.side === "left" ? -root.edgeHitBleed : 0
        anchors.rightMargin: root.side === "right" ? -root.edgeHitBleed : 0
        acceptedButtons: Qt.LeftButton
        hoverEnabled: true
        z: 1

        onPressed: function(mouse) {
            var point = root.pointInGraphCanvas(socketDot, socketDot.width / 2, socketDot.height / 2)
            root.connectionDragStarted(root.socketId, root.side, point.x, point.y)
            mouse.accepted = true
        }

        onPositionChanged: function(mouse) {
            if (!pressed) {
                return
            }

            var point = root.pointInGraphCanvas(socketDragArea, mouse.x, mouse.y)
            root.connectionDragged(point.x, point.y)
        }

        onReleased: function(mouse) {
            var point = root.pointInGraphCanvas(socketDragArea, mouse.x, mouse.y)
            root.connectionDragFinished(point.x, point.y)
        }
    }

    Label {
        id: socketLabel

        anchors.verticalCenter: parent.verticalCenter
        anchors.left: root.side === "left" ? socketDot.right : parent.left
        anchors.right: root.side === "right" ? socketDot.left : parent.right
        anchors.leftMargin: root.side === "left" ? 8 : 0
        anchors.rightMargin: root.side === "right" ? 8 : 0
        text: root.shownLabel
        color: colors.textMain
        font.pixelSize: 14
        horizontalAlignment: root.side === "left" ? Text.AlignLeft : Text.AlignRight
        enabled: false
        z: 0
    }
}
